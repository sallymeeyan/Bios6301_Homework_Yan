...
*Due Tuesday, 04 September, 1:00 PM*

50 points total.

Submit a single knitr file (named `homework1.rmd`) by email to coleman.r.harris@vanderbilt.edu.
...
pt(summary(mod)$coefficients[,3],7,lower.tail = F)*2
```
...
```{r}
(-3.8)^2+0.2^2+2.5^2+2.2^2+2.5^2+.5^2+1.2^2+0.5^2+0.2^2
...
```
The equal variances t-test matches the p-value from the original model.
